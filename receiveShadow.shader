    ...
    SubShader
    {
        Pass
        {
            ...
            fixed4 frag(v2f v):SV_Target
            {
                ...
                fixed3 texColor = tex2D(_MainTex, v.uv).rgb;
                fixed3 ambient = UNITY_LIGHTMODEL_AMBIENT * texColor;
                fixed3 diffuse = _LightColor0.rgb * texColor * saturate(dot(v.worldNormal, worldLightDir));
                fixed3 viewDir = normalize(UnityWorldSpaceViewDir(v.worldPosition));
                fixed3 halfDir = normalize(worldLightDir + viewDir);
                fixed3 specular = texColor * _LightColor0.rgb * _Specular.rgb * pow(
                    saturate(dot(v.worldNormal, halfDir)), _Gloss);
